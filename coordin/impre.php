...
<head>
 ...
 <link rel="shortcut icon" type="image/x-icon" href="../images/favicon.png">
  <title>Fovissste | IMPRESIONES </title>
...
<?php } 
  ...
  if (empty($_GET['area'])){
  $bareas = "";
  }else{ $bareas = $_GET["area"]; } 

  if (empty($_GET['prov'])){
  $fprovs = "";
  }else{ $fprovs = $_GET["prov"]; }

  if (empty($_GET['capi'])){
  $fcapi = "%";
  }else{ $fcapi = $_GET["capi"]; }

  if (empty($_GET['serv'])){
  $fserv = "";
  }else{ $fserv = $_GET["serv"]; }
?>
    ...
    <section class="content">
      ...
      <div class="card">
       ...
       <form id="form1" name="form1" method="post" action="">
        ...
        <?php do { echo '<option value= "'.$row["id"].'"';
        if (str_replace("%","",$bareas)==$row["id"]){ echo ("selected='selected'"); }
        ...
        if (str_replace("%","",$fcapi)==$rot["id"]){ echo ("selected='selected'"); }
        ...
        <thead>
        <tr class="headings"> <th class="column-title"># </th>
